    ...
    <div class="app-main__outer">
        ...
        <div class="app-main__inner mb-5">
            ...
            <div class="container-fulid border p-4 bg-white">
                ...
                <form action="<?= base_url('document/save_edit_dept/') . $get_fulldatas->dc_data_doccode; ?>" method="POST" name="form1" id="form1" enctype="multipart/form-data">
                    <!-- Form Section 1 -->
                    <!-- Get Form Code -->
                    ...
                    $get_doc_type_use = $this->db->query("SELECT
                        ...
                        FROM
                        dc_datamain
                        INNER JOIN dc_type_use ON dc_type_use.dc_type_use_darcode = dc_datamain.dc_data_darcode
            ...
            <div class="form-row">
                <?php foreach ($get_reason->result_array() as $rs_reason) { ?>
                    <!-- Reason request loop -->
                    <?php
                    if ($rs_reason['dc_reason_code'] == "r-03") {
                        $checked = ' checked="" ';
                    } else {
                        $checked = '';
                    }
            ...
            <div class="form-row mt-2">
                <label for="">เอกสารเดิม : &nbsp;</label><a href="<?= base_url() . $get_fulldatas->lib_main_file_location_copy . $get_fulldatas->lib_main_doccode_copy ?>#toolbar=0" target="_blank"><?= $get_fulldatas->dc_data_file; ?></a>
            </div>
            <div class="form-row mt-2">
                <div class="form-group">
                    <label for=""><?= label("uploadfile", $this); ?></label>
                    <input type="file" name="dc_data_file" id="dc_data_file" class="form-control" disabled>
                </div>
            </div>
